...
public class TradeServletAction {
    ...
    void doPortfolio(ServletContext ctx, HttpServletRequest req,
	    ...
	    XTraceContext.logEvent("FE End point", "RecvdReqFrmFE");
	    String xtrace = (XTraceContext.getThreadContext().pack()).toString();
	    XTraceContext.logEvent("FE End point", "SendingReqToBL");
            req.setAttribute("Xtrace",xtrace);

            Collection holdingDataBeans = tAction.getHoldings(userID);

	    XTraceContext.logEvent("FE End point", "SendingResponseToFE");
	    xtrace = (XTraceContext.getThreadContext().pack()).toString();
	    req.setAttribute("Xtrace",xtrace);
